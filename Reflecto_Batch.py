...
def Zyminterne(chemin_instal_Zyminterne, chemin_Images_input, chemin_parametres_Reflecto,
               chemin_parametres_statistiques, chemin_Reconstruction, chemin_Synthese, longueur_onde,
    ...
    commande2 = r'Zyminterne.exe -layer -i ' + chemin_Images_input + ' -f ' + chemin_Reconstruction + ' -o ' + chemin_Synthese + ' -s ' + chemin_parametres_statistiques + ' -r ' + chemin_parametres_Reflecto + ' -d ' + images_reconstruction
...
def selection_fichier_parametres_Reflecto():
    global chemin_Reflecto
    fichier_selectionne = filedialog.askopenfilename(title="Reflecto", filetypes=[("Fichiers texte", "*.txt"),
                                                                                  ("Tous les fichiers", "*.*")])
    chemin_Reflecto = fichier_selectionne
    if chemin_Reflecto != '':
        bouton_Reflecto['bg'] = 'green'
...
def lancer_reconstruction():
    threading.Thread(
        target=reconstructions(chemin_variation_param, chemin_instal_Zyminterne, chemin_dossier_acquisitions,
                               chemin_Reflecto, chemin_Stat, images_reconstruction, case_POST.get(), case_EXPO.get(),
                               case_plate_type.get())).start()


def reconstructions(chemin_variation_param, chemin_instal_Zyminterne, chemin_dossier_acquisitions, chemin_Reflecto,
    ...
    if nb_variation_max[1] > 1:
        ...
        if if_colonne_plate_type:
            ...
            print('Variation de paramètres Reflecto et/ou Stat : ', compteur, ' conditions à tester')
    else:
        compteur = 1
        print('Aucune variation des paramètres Reflecto et Stat')
    ...
    if not variation:
        messagebox.showerror("Information", "Aucune variation des paramètres Reflecto et Stat\n" + str(
    ...
    chemin_parametres_Reflecto = inverser_slash(chemin_Reflecto)
    chemin_parametres_statistiques = inverser_slash(chemin_Stat)

    longueur_onde_boucle = ''
    with open(chemin_parametres_Reflecto, 'r') as fichier_Reflecto:
        lignes_Reflecto = fichier_Reflecto.read()
    parametres = lignes_Reflecto.split()
    ...
    if variation:
        ...
        for condition_test in range(len(conditions)):
            for acquisition in range(len(liste_acquisitions)):
                for PE in pe:
                    ...
                    # détection des paramètres Reflecto ou Stat à modifier
                    param_Reflecto = []
                    param_Stat = []
                    for i in range(len(conditions[condition_test])):
                        if conditions[condition_test][i][0] in parametres_reflecto:
                            param_Reflecto.append(conditions[condition_test][i])
                    ...
                    # génération fichier Reflecto
                    if len(param_Reflecto) != 0:
                        # Lire le contenu du fichier original
                        with open(chemin_parametres_Reflecto, 'r') as fichier_Reflecto:
                            lignes_Reflecto = fichier_Reflecto.read()
                        parametres = lignes_Reflecto.split()
                        for k in range(len(param_Reflecto)):
                            for j in range(len(parametres) - 1):
                                # Modifier le paramètre souhaité
                                if '-' + param_Reflecto[k][0] == parametres[j]:
                                    if param_Reflecto[k][0] in parametres_int:
                                        parametres[j + 1] = str(int(param_Reflecto[k][1]))
                                    else:
                                        parametres[j + 1] = str(param_Reflecto[k][1])
                        # Écrire le contenu modifié dans un nouveau fichier
                        nom_new_fichier_reflecto = '\\Parametrage\\Reflecto_'
                        for i in range(len(param_Reflecto)):
                            if param_Reflecto[i][0] in parametres_int:
                                nom_new_fichier_reflecto += param_Reflecto[i][0] + '=' + str(
                                    int(param_Reflecto[i][1])) + '_'
                            else:
                                nom_new_fichier_reflecto += param_Reflecto[i][0] + '=' + str(param_Reflecto[i][1]) + '_'
                        nom_new_fichier_reflecto = remplacer_point_virgule(nom_new_fichier_reflecto[:-1]) + '.txt'
                        nouveau_contenu = " ".join(parametres)
                        with open(chemin_Reconstruction + nom_new_fichier_reflecto, 'w') as fichier:
                            fichier.write(nouveau_contenu)
                        chemin_parametres_Reflecto = chemin_Reconstruction + nom_new_fichier_reflecto
                    ...
                    Zyminterne(chemin_instal_Zyminterne, GifS(chemin_Images_input), GifS(chemin_parametres_Reflecto),
        ...
        for acquisition in range(len(liste_acquisitions)):
            for PE in pe:
                chemin_Images_input = inverser_slash(chemin_dossier_acquisitions) + '\\' + liste_acquisitions[
                    acquisition, 0] + '\\' + PE + '\\Images'
                nom_dossier_reconstruction = 'reconstruction_test_calib_free_restreinte_7_%'
                chemin_Reconstruction = inverser_slash(chemin_dossier_acquisitions) + '\\' + liste_acquisitions[
                    acquisition, 0] + '\\' + PE + '\\' + nom_dossier_reconstruction
                if not os.path.exists(chemin_Reconstruction):
                    os.makedirs(chemin_Reconstruction)
...
checkbox_EXPO.pack()
bouton_Reflecto = tk.Button(fenetre, text="Reflecto", command=selection_fichier_parametres_Reflecto)
bouton_Reflecto.pack(pady=20)
